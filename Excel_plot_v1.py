...
# Data and Colors (Define multiple plots in a structured format)
plots = [
    ...
    {"data": da1_c2_left, "x": "M_min", "y": "Y", "color": "red", "label": "M_min vs Y - C2 Left"},
    {"data": da1_c2_left, "x": "M_max", "y": "Y", "color": "purple", "label": "M_max vs Y - C2 Left"}
    # Add more entries here to add additional plots
]
...
# Plot each entry in the plots list
for plot in plots:
    ax.plot(plot["data"][plot["x"]], plot["data"][plot["y"]],
            color=plot["color"], label=plot["label"])

# Update x-axis limits dynamically based on all data ranges
x_min = min([plot["data"][plot["x"]].min() for plot in plots])
x_max = max([plot["data"][plot["x"]].max() for plot in plots])
